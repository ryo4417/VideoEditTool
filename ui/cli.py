...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("input", nargs="+",
                        help="入力動画ファイル（複数指定・ワイルドカード可。1プロセスで連続処理しモデルを使い回す）")
...
def main(argv: list[str] | None = None) -> int:
    ...
    # 入力を展開（ワイルドカード対応）。同一プロセスで回すのでWhisperモデルは使い回される。
    inputs: list[str] = []
    ...
    for path in inputs:
        try:
            result = pipe.analyze(path)  # 解析は入力ごとに1回だけ
            outputs: list[str] = []
            ...
            print(f"[エラー] {path}: {e}", file=sys.stderr)
            exit_code = 2
            continue
