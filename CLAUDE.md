...
- 実装済み（main）:
  ...
  - **GUI手編集**: タイムラインでカットを移動/伸縮/削除/追加（ドラッグ）、一覧で秒数微調整、編集後プレビュー、進捗表示。
  - **ファイル投入**: ドラッグ&ドロップ/選択/お試し動画/パスのクォート自動除去。**Google Drive**（デスクトップ版）入出力。
  - CLI: 複数入力(ワイルドカード)・複数形式1回解析・`--transcript`・UTF-8出力。`start.bat`でダブルクリック起動。
  - テスト件数は `python -m pytest` を参照（CIでffmpeg込み実行）。
- 保留: 案件マニュアル学習（マニュアル未提供）、真のクロスフェード/カット境界スナップ（実素材での調整が必要）。
